...
contract OwnableWithDAO {
    ...
    //Модификация для защиты от вызова не создателя контракта
    modifier onlyOwner() {
        require(msg.sender == owner);
        _;
    }

    //Модификация для защиты от вызова не со стороны DAO
    modifier onlyDAO() {
...
contract Stoppable is OwnableWithDAO {
    ...
    //Модификатор для проверки возможности выполнения фунуции
    modifier stoppable {
        require(!stopped);
        _;
    }
...
contract DAOToken is Stoppable {
    ...
    /* function buyPriceCheck() public returns (uint) {
        buyPrice;
    } */

    // Внутренняя функция для перевода токенов
    function _transfer(address _from, address _to, uint256 _value) internal stoppable {
        require(_to != 0x0);
    ...
    function _buy(address _from, uint _value) internal {
      ...
      //Вызываем функцию для перевода токенов
        _transfer(this, _from, amount);
    } */

    //Функция для смены названия токена
    function changeSymbol(string _symbol) public onlyDAO {
        symbol = _symbol;
    }
